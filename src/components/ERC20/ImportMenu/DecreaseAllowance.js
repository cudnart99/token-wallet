...
const DecreaseAllowance = ({ web3Token, refreshDataGrid, tokenData }) => {
  ...
  const onClickIncrease = async () => {
    ...
    try {
      ...
      await web3Token.methods
        .decreaseAllowance(data.arg1, amountToApprove)
        .send({ from: accounts[0] });
      successMessage = `Decrease allowance successful. ${data.arg2} ${symbol} decreased from ${data.arg1}`;
      ...
      <Grid item xs={12}>
        ...
        >
          {data.loading ? (
            ...
            "Decrease Allowance (address spender, uint256 value)"
          )}
        </Button>
      </Grid>
      <Grid item xs={12}>
